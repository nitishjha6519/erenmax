...
export interface GoalSession {
  ...
  approvedHelper?: { id: string; name: string; avatar?: string; trustScore?: number }
...
export interface Session {
  ...
  repsLeft?: number
  stakedPoints?: number
  goal?: { id: string; title: string; category: string; description?: string; difficulty?: string; pledgedPoints?: number }
}
...
export const api = {
  ...
  users: {
    ...
    getMyStats: () =>
      request<{
        stats: UserStats
        scoreBreakdown?: ScoreBreakdown
        weeklyActivity?: Array<{ date: string; sessions: number; points: number }>
        categoryBreakdown?: Array<{ category: string; sessionsCount: number; points: number; percentage: number }>
      }>('/api/users/me/stats'),
  ...
  applications: {
    apply: (sessionId: string, data: { message?: string; stakedPoints: number }) =>
  ...
  sessions: {
    ...
    open: (params?: { category?: string; from?: string; limit?: number; offset?: number }) =>
    ...
    getLive: (sessionId: string) =>
      request<{ isLive: boolean; session: LiveSession }>(`/api/sessions/${sessionId}/live`),
    update: (
      sessionId: string,
