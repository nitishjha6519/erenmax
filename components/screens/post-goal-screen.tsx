...
export function PostGoalScreen({ onNavigate }: PostGoalScreenProps) {
  ...
  return (
    <div className="animate-fade-up">
      <div className="font-mono text-[11px] text-text3 tracking-[0.12em] uppercase mb-1.5">New Goal</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 max-w-[860px]">
        ...
        <div>
          ...
          {currentStep === 1 && (
            <div>
              <div className="font-mono text-[11px] text-text3 mb-4">Step 1 of 3 — Your goal</div>
              <div className="mb-4 md:mb-5">
                <label className="block text-[13px] font-medium text-text2 mb-1.5 font-mono">Goal title</label>
                <input 
              ...
              <div className="mb-4 md:mb-5">
                <label className="block text-[13px] font-medium text-text2 mb-1.5 font-mono">What types of sessions will you need?</label>
                  ...
                  {['DSA', 'System Design', 'Behavioral'].map((type) => (
                    <button
                      ...
                      className={cn(
                        ...
                        selectedTypes.includes(type) && "border-primary text-primary bg-[#fff5f2]"
                      )}
                    >
                      {type}
                    </button>
                ...
                <div className="font-mono text-[10px] md:text-[11px] text-text3 mt-2">You&apos;ll add specific topics from your dashboard after posting.</div>
              </div>
              <div className="mb-4 md:mb-5">
                <label className="block text-[13px] font-medium text-text2 mb-1.5 font-mono">Target company / role</label>
                <input 
                  className="w-full px-3.5 py-2.5 border-[1.5px] border-border rounded-lg text-sm md:text-[15px] bg-card focus:border-primary outline-none"
          ...
          {currentStep === 2 && (
            <div>
              <div className="font-mono text-[11px] text-text3 mb-4">Step 2 of 3 — Settings</div>
              ...
              <div className="flex gap-2">
                ...
                <Button className="flex-1" onClick={() => setCurrentStep(3)}>Continue</Button>
              </div>
            </div>
          )}
          ...
          {currentStep === 3 && (
            <div>
              <div className="font-mono text-[11px] text-text3 mb-4">Step 3 of 3 — Review & post</div>
              <div className="bg-card border border-border rounded-[20px] p-4 md:p-6 mb-4">
                <div className="font-display font-bold text-sm md:text-base mb-2.5">{goalTitle || 'Crack FAANG in 10 weeks'}</div>
                <div className="flex gap-2 flex-wrap mb-3">
                  {selectedTypes.map((type) => (
                    <span key={type} className={`text-[11px] font-mono font-medium px-2.5 py-0.5 rounded-full ${type === 'DSA' ? 'badge-dsa' : type === 'System Design' ? 'badge-design' : 'badge-behavioral'}`}>
                      {type}
                    </span>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <div>
                    <div className="font-mono text-[10px] md:text-[11px] text-text3">Session length</div>
                    <div className="font-semibold text-xs md:text-sm text-foreground mt-1">45 mins (default)</div>
                  </div>
                  <div>
                    <div className="font-mono text-[10px] md:text-[11px] text-text3">Approval deadline</div>
                    <div className="font-semibold text-xs md:text-sm text-foreground mt-1">6h before</div>
                  </div>
                  <div>
                    <div className="font-mono text-[10px] md:text-[11px] text-text3">Platform</div>
                    <div className="font-semibold text-xs md:text-sm text-foreground mt-1">Google Meet</div>
                  </div>
                  <div>
                    <div className="font-mono text-[10px] md:text-[11px] text-text3">Topics added</div>
                    <div className="font-display font-extrabold text-primary mt-1">0 (add after)</div>
                  </div>
                </div>
        ...
        <div className="hidden lg:block">
          <div className="font-mono text-[11px] text-text3 mb-4">How it looks to the community</div>
          <div className="bg-card border border-border rounded-[20px] p-5 md:p-6 mb-4">
            ...
            <div className="text-sm text-text2 mb-4">This is shown publicly — be honest, it attracts helpers who take it seriously.</div>
          ...
          <div className="bg-surface rounded-[20px] p-5">
            <div className="font-mono text-[11px] text-text3 mb-3">What happens after posting</div>
            <div className="relative pl-6">
              <div className="absolute left-[7px] top-2 bottom-2 w-px bg-border" />
              {[
                { done: true, active: false, title: 'Goal posted', desc: 'Visible to community. No open slots yet.' },
                { done: false, active: true, title: 'Add topics in Dashboard', desc: 'Each topic = one session slot with a date/time. Community can now see and apply.' },
                { done: false, active: false, title: 'Applicants come in', desc: 'You review each person\'s trust score and message. Approve the best one per slot.' },
                { done: false, active: false, title: 'Sessions run, reps count', desc: 'Each completed session = 1 rep. Reach 100.' },
              ].map((item, i) => (
                <div key={i} className="relative pb-5 pl-5">
                  ...
                  <div className="font-mono text-[11px] text-text3 mt-1">{item.desc}</div>
                </div>
              ))}
            </div>
          </div>
