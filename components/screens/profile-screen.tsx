...
export function ProfileScreen() {
  ...
  const saveProfile = async () => {
    setSaving(true);
    try {
      await api.users.updateProfile({ bio });
    } catch (e) {
      console.error(e);
    } finally {
      setSaving(false);
  ...
  const yoe = user?.yoe;
  const showUpPct =
    stats?.showUpRate != null ? `${Math.round(stats.showUpRate * 100)}%` : "—";
  ...
  return (
    <div className="animate-fade-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-6 max-w-[960px]">
          ...
          <div className="bg-card border border-border rounded-[20px] p-4 md:p-6 mb-4">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-4 md:mb-5">
              <div className="relative self-start">
                <div className="w-12 md:w-14 h-12 md:h-14 rounded-full av-orange flex items-center justify-center font-display font-extrabold text-base md:text-lg">
                  {initials}
                </div>
                <div
                  className="absolute bottom-0 right-0 w-5 h-5 bg-primary rounded-full border-2 border-card flex items-center justify-center text-[10px] text-white cursor-pointer"
              ...
              <div className="flex-1">
                <div className="font-display font-extrabold text-lg md:text-xl">
                  {name}
                </div>
                <div className="font-mono text-[10px] md:text-[11px] text-text3 mt-1">
                  {[location, yoe ? `${yoe} YOE` : null]
                    .filter(Boolean)
                    .join(" · ")}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-2 text-center">
              <div>
                <div className="font-display font-extrabold text-xl md:text-[28px] text-green tracking-tight">
          ...
          <div className="bg-ink rounded-[20px] p-4 md:p-6 mb-4">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="text-center sm:text-left">
                <div className="font-display font-extrabold text-4xl md:text-5xl text-green tracking-tight leading-none">
                  {stats?.trustScore ?? 500}
                </div>
                <div className="font-mono text-[10px] text-white/40 mt-1 uppercase tracking-wider">
                  Trust score
                </div>
              </div>
              <div className="flex-1">
                ...
                <div className="flex justify-between text-xs mb-1.5">
                  <span className="text-white/50">Show-up rate</span>
                  <span className="text-green font-mono">{showUpPct}</span>
                </div>
                <div className="flex justify-between text-xs mb-1.5">
                  <span className="text-white/50">Avg rating</span>
                  <span className="text-green font-mono">{avgRating}</span>
                </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-white/60">
                {(stats?.trustScore ?? 0) >= 750
                  ? "750+ · Highly reliable · Shown first in open slots"
                  : (stats?.trustScore ?? 0) >= 500
                    ? "500–749 · Reliable. Occasionally missed sessions."
                    : "Below 500 · Risky. Shown last in feed."}
              </span>
            </div>
          </div>
        </div>
        ...
        <div>
          {/* Sessions helped */}
          <div className="font-display font-bold text-sm mb-3 md:mb-4">
          ...
          {loading ? (
            <div className="flex flex-col gap-2">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="bg-surface rounded-xl p-3 animate-pulse"
                >
            ...
            <p className="text-sm text-text3 mb-6">
              No sessions helped yet. Browse the feed to start helping others.
            </p>
          ) : (
                ...
                <div key={s.id} className="bg-surface rounded-xl p-3 md:p-4">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                    <div>
                      ...
                      <div className="font-mono text-[10px] md:text-[11px] text-text3 mt-1">
                        {s.goal?.title ? `${s.goal.title} · ` : ""}
                        {formatDate(
                          s.scheduledAt ?? s.scheduledDate ?? "",
                        )} · {s.duration ?? 60} min
                      </div>
                    </div>
                    <span
                      className={`${catBadgeClass(s.category)} text-[10px] font-mono font-medium px-2 py-0.5 rounded-full self-start sm:self-center`}
                    >
                      {catLabel(s.category)}
                    </span>
                  </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs md:text-sm">
                  <strong>750+</strong> — Highly reliable, shown first in open
                  slots
                </span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-2 h-2 rounded-full bg-[#c17800]" />
                <span className="text-xs md:text-sm">
                  <strong>500–749</strong> — Reliable. Occasionally missed
                  sessions.
                </span>
              </div>
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs md:text-sm">
                  <strong>Below 500</strong> — Risky. Shown last in feed.
                </span>
              </div>
            </div>
          </div>
